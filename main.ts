...
let lightvsdark = game.askForNumber("Type 0 for dark mode and 1 for light mode")
if (lightvsdark == 0) {
...
if (lightvsdark == 0) {
    ...
    for (let index = 0; index <= 15; index++) {
        for (let value = 0; value <= 15; value++) {
            if (Math.percentChance(15) && tiles.tileAtLocationEquals(tiles.getTileLocation(index, value), assets.tile`myTile1`) && index + value > 3) {
                tiles.setTileAt(tiles.getTileLocation(index, value), assets.tile`myTile0`)
            }
        }
    }
